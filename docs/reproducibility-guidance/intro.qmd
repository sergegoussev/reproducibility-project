...
-   **Input data** is the data used to answer research questions. It may be open, closed (proprietary or sensitive), or synthetic. Each is approached separately. Some notable aspects:
    ...
    -   Ideally open data is used as this ensures the research can be fully reproducible and the dataset used can act as a benchmark if other methods are tried on it. We list a few available open datasets in the [discipline specific catalogue](https://un-task-team-for-scanner-data.github.io/price-stats-data-catalogue/).

    -   While input data is ingested and processed by the research, in itself it is *ignored* by the research compendium (such as by listing the data files in the `.gitingore`). Read more in the [input data guide](/docs/reproducibility-guidance/howtos/input-data.qmd).

-   **Ouput data** is data created as part of the research process. Whether (and how) its shared depends on various circumstances and value to later research projects (to say act as an input dataset for other projects). Read [about this process here](/docs/reproducibility-guidance/howtos/opening-data.qmd).
...
-   Its logical **structure** ensures maturity and helps others (or you later) in trying to peer review, reproduce or extend the research. Read more about how and why to set up an appropriate structure [here](/docs/reproducibility-guidance/howtos/research-compendium-structure.qmd).
...
Use of repositories such as Zenodo depends on the researchers and the organization they are in. It is best to follow your organization's rules for which repository it is best to publish from. The main idea is to publish to a repository that helps create a DOI which is (ideally) immunable.
:::
...
-   If the code is pivotal to the methodology (for instance the package was used to calculate price indices), then it is best to acknowledge this in the paper ([by citing it](/docs/reproducibility-guidance/howtos/citing-code.qmd)).
